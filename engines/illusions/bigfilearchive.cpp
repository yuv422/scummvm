...
byte *BigfileArchive::readResource(uint32 sceneId, uint32 resId, uint32 &dataSize) {
	uint32 id = resId & 0xffff;
	if (id >= TOTAL_FILES) {
	...
	for(uint32 i=0; i < TOTAL_FILES; i++) {
		if (strcmp(fileInfo[i].filename, filename) == 0) {
			...
			if (strcmp(extension, "act") == 0) {
				return ILLUSIONS_DRAGONS_ACT | i;
			} else if (strcmp(extension, "scr") == 0) {
				return ILLUSIONS_DRAGONS_SCR | i;
			} else if (strcmp(extension, "rms") == 0) {
				return ILLUSIONS_DRAGONS_RMS | i;
			}
		}
	}
	return 0;
